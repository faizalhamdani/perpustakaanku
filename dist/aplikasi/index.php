                    ...
                    <div class="col-xl-4 col-lg-5">
                        ...
                        <!-- Card Header - Dropdown -->
                            ...
                            <?php 
                                ...
                                $hasil=mysqli_query($kon,"select * from kebijakan limit 1");
                                $data = mysqli_fetch_array($hasil); 
                            ?>
                            <!-- Card Body -->
                            <div class="card-body">
                                <form action="aplikasi/edit.php" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        <label>Estimasi waktu Peminjaman: <span id="info_waktu_pinjam"><?php if (isset( $data['max_wkt_pjm'])) echo $data['max_wkt_pjm'];?> hari</span> </label>
                                        <input type="text" class="form-control" id="max_wkt_pjm" value="<?php if (isset( $data['max_wkt_pjm'])) echo $data['max_wkt_pjm'];?>" name="max_wkt_pjm" required>  
                                    </div>
                                    <div class="form-group">
                                        <label>Maksimal Peminjaman: <span id="info_maksimal_pinjam"><?php if (isset( $data['max_jml_koleksi'])) echo $data['max_jml_koleksi'];?> Pustaka</span> </label>
                                        <input type="text" class="form-control" id="maksimal_pinjam" value="<?php if (isset( $data['max_jml_koleksi'])) echo $data['max_jml_koleksi'];?>" name="max_jml_koleksi" required>  
                                    </div>
                                    <div class="form-group">
                                        <label>Besaran Denda Keterlambatan: <span id="info_denda_terlambat">Rp. <?php if (isset( $data['denda'])) echo number_format($data['denda'],0,',','.'); ?>/hari</span> </label>
                                        <input type="text" class="form-control" id="denda_terlambat" value="<?php if (isset( $data['denda'])) echo $data['denda'];?>" name="denda" required>  
                                    </div>
                                    <div class="form-group">
                                        <button type="submit" class="btn btn-primary"name="ubah_kebijakan" >Simpan Perubahan</button>
                                    </div>
                                </form>
                            </div>
                        </div>
...
<script>
    ...
    $('#max_wkt_pjm').bind('keyup', function () {
        var max_wkt_pjm=$("#max_wkt_pjm").val();
        $("#info_waktu_pinjam").text(max_wkt_pjm+' hari');     
    }); 
